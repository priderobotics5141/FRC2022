  ...
  public void teleopPeriodic() {
    if(gamePad.getLeftX() >= 0.1 || gamePad.getLeftX() <= -0.1 ||  gamePad.getLeftY() >=0.1 || gamePad.getLeftY() <=-0.1) {
      ...
      //rotate
        ...
        double multiplier = Math.cos((2.0*Math.PI/512.0)*((Robot.three.getSelectedSensorPosition())));

        falcRight.set(TalonSRXControlMode.PercentOutput, (gamePad.getRightTriggerAxis() * 0.4) - (gamePad.getRightX() * 0.2 * (1-multiplier)));
        falcLeft.set(TalonSRXControlMode.PercentOutput, (gamePad.getRightTriggerAxis() * 0.4) + (gamePad.getRightX() * 0.2 * (1-multiplier)));
