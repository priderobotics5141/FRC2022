...
public class Robot extends TimedRobot {
    ...
    //if (pidFalcLeftDistance.getPositionError() <= 3000) {
      ...
      } else 
      {
        falcLeft.set(ControlMode.PercentOutput,
          pidFalcLeftDistance.calculate(falcLeft.getSelectedSensorPosition(), -110000) * 0.7); // 36 inches * (10000/9)=40000
      falcRight.set(ControlMode.PercentOutput,
          pidFalcRightDistance.calculate(falcRight.getSelectedSensorPosition(), -110000) * 0.7);
      }
  ...
  public void teleopPeriodic() {
    ...
    if (gamePad.getRightBumper()) {
      falcShooter1.set(TalonFXControlMode.PercentOutput, 0.7);
      falcShooter2.set(TalonFXControlMode.PercentOutput, 0.7);
    }
